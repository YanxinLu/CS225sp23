...
class KDTree
{
  ...
  private:
    ...
    /* Helper function for partition of quickselect*/
    ...
    /* Helper recursive function for contructor*/
    ...
    Point<Dim> neighborsearch(KDTreeNode* subroot, const Point<Dim>& point, int d) const{
      ...
      int dist_sq = (subroot->point[d]-point[d])*(subroot->point[d]-point[d]);
      int radius_sq = 0;
      for(int i = 0; i<Dim; i++){
        radius_sq+=(closest[i]-point[i])*(closest[i]-point[i]);
      }
