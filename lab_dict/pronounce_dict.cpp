...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    auto find1 = dict.find(up1);
    auto find2 = dict.find(up2);
    if(find1!=dict.end()&&find2!=dict.end()){
        vector<string> pron1 = find1->second;
        vector<string> pron2 = find2->second;
        if(pron1.size()!=pron2.size()) return false;
        for(size_t i = 0; i <pron1.size(); i++){
            if(pron1[i]!=pron2[i]) return false;
        }
        return true;
    }
    return false;
