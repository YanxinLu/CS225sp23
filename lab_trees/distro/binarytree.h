...
class BinaryTree
{
  private:
    ...
    //helper functions
    void r_mirror(Node* node);
    bool r_ordered(Node* node, const T* max, const T* min)const;
    void r_paths(vector<vector<T> > &paths, Node* node, int curr)const;
    int r_sumdis(Node* node, int subpath)const;
  public:
    /**
