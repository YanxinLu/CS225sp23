...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(true){
        ...
        TreeNode* newmerge = new TreeNode(left->freq.getFrequency() + right->freq.getFrequency());
        newmerge->left = left;
        newmerge->right = right;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.hasBits()){
        if(bfile.getNextBit()){
            Frequency leaf_fre(bfile.getNextByte(),0);
            TreeNode* leaf = new TreeNode(leaf_fre);
            node = leaf;
        }
        else{
            TreeNode* itn = new TreeNode(0);
