...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if(elements[left]>val||elements[left]==val) return 0;
    if(elements[right]==val) return right;
    if(elements[right]<val) return elements.size(); 
    while(true){
        int middle = (left+right)/2;
        if(elements[middle]==val) return middle;
        if(elements[middle]<val){
            if(middle+1==right) return right;
            left = middle;
        }
        if(elements[middle]>val){
            if(left+1==middle) return middle;
            right = middle;
        }
    }
    return 0;
